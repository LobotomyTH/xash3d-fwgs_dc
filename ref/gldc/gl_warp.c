...
static void ClipSkyPolygon( int nump, vec3_t vecs, int stage )
{
	...
	if( nump > MAX_CLIP_VERTS )
		gEngfuncs_gl.Host_Error( "%s: MAX_CLIP_VERTS\n", __func__ );
loc1:
	if( stage == 6 )
	{
...
#if !XASH_DREAMCAST
	if( RI.fogEnabled )
		glFogf( GL_FOG_DENSITY, RI.fogDensity * 0.5f );
#endif
	glDisable( GL_BLEND );
...
#if !XASH_DREAMCAST
	if( RI.fogEnabled )
		glFogf( GL_FOG_DENSITY, RI.fogDensity * 0.25f );
#endif
...
qboolean R_UploadRipples( texture_t *image )
{
	...
	glTexImage2D( GL_TEXTURE_2D, 0, glt->format, width, height, 0,

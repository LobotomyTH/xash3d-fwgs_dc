...
static void FracNoise( float *noise, int divs )
{
	...
	noise[div2] = ( noise[0] + noise[divs] ) * 0.5f + divs * gEngfuncs_gl.COM_RandomFloat( -0.125f, 0.125f );
...
qboolean R_BeamCull( const vec3_t start, const vec3_t end, qboolean pvsOnly )
{
	...
	// check bbox
	if( gEngfuncs_gl.Mod_BoxVisible( mins, maxs, Mod_GetCurrentVis( )))
	{
...
void CL_AddCustomBeam( cl_entity_t *pEnvBeam )
{
	...
	{
		gEngfuncs_gl.Con_Printf( S_ERROR "Too many beams %d!\n", tr.draw_list->num_beam_entities );
		return;
	}
...
static void R_DrawBeamFollow( BEAM *pbeam, float frametime )
{
	...
	gEngfuncs_gl.R_FreeDeadParticles( &pbeam->particles );
	...
	if( FBitSet( pbeam->flags, FBEAM_STARTENTITY ))
	{
		if( particles )
		{
			...
			if( div >= 32 )
			{
				pnew = gEngfuncs_gl.CL_AllocParticleFast();
			}
		}
		else
		{
...
static void R_DrawRing( vec3_t source, vec3_t delta, float width, float amplitude, float freq, float speed, int segments )
{
	...
	if( !gEngfuncs_gl.Mod_BoxVisible( screen, tmp, Mod_GetCurrentVis( )) || R_CullBox( screen, tmp ))
	{
		return;
	}
...
static qboolean R_BeamComputePoint( int beamEnt, vec3_t pt )
{
	...
	ent = gEngfuncs_gl.R_BeamGetEntity( beamEnt );
	...
	if( !ent )
	{
		gEngfuncs_gl.Con_DPrintf( S_ERROR "%s: invalid entity %i\n", __func__, BEAMENT_ENTITY( beamEnt ));
		VectorClear( pt );
		return false;
	}
...
static qboolean R_BeamRecomputeEndpoints( BEAM *pbeam )
{
	if( FBitSet( pbeam->flags, FBEAM_STARTENTITY ))
	{
		cl_entity_t *start = gEngfuncs_gl.R_BeamGetEntity( pbeam->startEntity );
	...
	{
		cl_entity_t *end = gEngfuncs_gl.R_BeamGetEntity( pbeam->endEntity );
...
static void R_BeamDraw( BEAM *pbeam, float frametime )
{
	...
	if( pbeam->type == TE_BEAMFOLLOW )
	{
		...
		pStart = gEngfuncs_gl.R_BeamGetEntity( pbeam->startEntity );

...
void R_PushScene( void )
{
	if( ++tr.draw_stack_pos >= MAX_DRAW_STACK )
		gEngfuncs_gl.Host_Error( "draw stack overflow\n" );
...
void R_PopScene( void )
{
	if( --tr.draw_stack_pos < 0 )
		gEngfuncs_gl.Host_Error( "draw stack underflow\n" );
	tr.draw_list = &tr.draw_stack[tr.draw_stack_pos];
}
...
void R_ClearScene( void )
{
	...
	if( gEngfuncs_gl.drawFuncs->R_ClearScene != NULL )
		gEngfuncs_gl.drawFuncs->R_ClearScene();
...
{
	const ref_overview_t	*ov = gEngfuncs_gl.GetOverviewParms();
...
void R_FindViewLeaf( void )
{
	...
	RI.viewleaf = gEngfuncs_gl.Mod_PointInLeaf( RI.pvsorigin, WORLDMODEL->nodes );
}
...
static void R_SetupFrame( void )
{
	...
	glState.isFogEnabled = glIsEnabled( GL_FOG );
...
void R_SetupGL( qboolean set_gl_state )
{
	...
	if( RP_NORMALPASS( ))
	{
		...
		x = floor( RI.viewport[0] * gpGlobals_gl->width / gpGlobals_gl->width );
		x2 = ceil(( RI.viewport[0] + RI.viewport[2] ) * gpGlobals_gl->width / gpGlobals_gl->width );
		y = floor( gpGlobals_gl->height - RI.viewport[1] * gpGlobals_gl->height / gpGlobals_gl->height );
		y2 = ceil( gpGlobals_gl->height - ( RI.viewport[1] + RI.viewport[3] ) * gpGlobals_gl->height / gpGlobals_gl->height );
...
static void R_CheckFog( void )
{
	...
	ent = gEngfuncs_gl.CL_GetWaterEntity( RI.vieworg );
...
static void R_CheckGLFog( void )
{
	...
	if(( !RI.fogEnabled && !RI.fogCustom ) && glIsEnabled( GL_FOG ) && VectorIsNull( RI.fogColor ))
	{
		// fill the fog color from GL-state machine
		glGetFloatv( GL_FOG_COLOR, RI.fogColor );
		RI.fogSkybox = true;
	}
#endif
}
...
void R_DrawFog( void )
{
	...
	else glFogi( GL_FOG_MODE, GL_EXP );
...
static void R_DrawEntitiesOnList( void )
{
	...
	{
		gEngfuncs_gl.CL_DrawEFX( tr.frametime, false );
	}
	GL_CheckForErrors();

	if( RI.drawWorld )
		gEngfuncs_gl.pfnDrawNormalTriangles();
	GL_CheckForErrors();
	...
	if( RI.drawWorld )
	{
		...
		gEngfuncs_gl.pfnDrawTransparentTriangles ();
	}
	GL_CheckForErrors();
	...
	{
		R_AllowFog( false );
		gEngfuncs_gl.CL_DrawEFX( tr.frametime, true );
		R_AllowFog( true );
	}
	GL_CheckForErrors();
...
{
	if( !WORLDMODEL && RI.drawWorld )
		gEngfuncs_gl.Host_Error( "%s: NULL worldmodel\n", __func__ );
	...
	gEngfuncs_gl.CL_ExtraUpdate ();	// don't let sound get messed up if going slow
	R_DrawEntitiesOnList();
	R_DrawWaterSurfaces();
	R_EndGL();
}
...
void R_RenderFrame( const ref_viewpass_t *rvp )
{
	...
	if( gEngfuncs_gl.drawFuncs->GL_RenderFrame != NULL )
	{
		tr.fCustomRendering = true;

		if( gEngfuncs_gl.drawFuncs->GL_RenderFrame( rvp ))
		{
			R_GatherPlayerLight();
			tr.realframecount++;
			tr.fResetVis = true;
...
int CL_FxBlend( cl_entity_t *e )
{
	...
	case kRenderFxDistort:
		...
		else
		{
			...
			blend += gEngfuncs_gl.COM_RandomLong( -32, 31 );
		}
		break;
	default:
		blend = e->curstate.renderamt;

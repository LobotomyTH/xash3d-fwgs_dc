...
*/
#if XASH_DREAMCAST
void DC_Mod_LoadSpriteModel( model_t *mod, const void *buffer, qboolean *loaded, uint texFlags )
#else
...
#endif
{
	...
	if( numi == NULL )
	{
		...
		pal = gEngfuncs_gl.FS_LoadImage( "#id.pal", (byte *)&i, 768 );
		pframetype = ((const byte*)buffer + sizeof( dsprite_q1_t )); // pinq1 + 1
		gEngfuncs_gl.FS_FreeImage( pal ); // palette installed, no reason to keep this data
	}
	else if( *numi == 256 )
	{
		...
		{
		case SPR_INDEXALPHA:
			pal = gEngfuncs_gl.FS_LoadImage( "#gradient.pal", src, 768 );
			break;
		case SPR_ALPHTEST:
			pal = gEngfuncs_gl.FS_LoadImage( "#masked.pal", src, 768 );
			break;
		default:
			pal = gEngfuncs_gl.FS_LoadImage( "#normal.pal", src, 768 );
			break;
		}
	...
	{
		gEngfuncs_gl.Con_DPrintf( S_ERROR "%s has wrong number of palette colors %i (should be 256)\n", mod->name, *numi );
		return;
	}
...
void Mod_LoadMapSprite( model_t *mod, const void *buffer, size_t size, qboolean *loaded )
{
	...
	gEngfuncs_gl.Image_SetForceFlags( IL_OVERVIEW );
	pix = gEngfuncs_gl.FS_LoadImage( texname, buffer, size );
	gEngfuncs_gl.Image_ClearForceFlags();
	...
	gEngfuncs_gl.Image_Process( &pix, w, h, IMAGE_FORCE_RGBA|IMAGE_RESAMPLE, 0.0f );
	...
	temp.size = w * h * gEngfuncs_gl.Image_GetPFDesc(temp.type)->bpp;
...
mspriteframe_t *R_GetSpriteFrame( const model_t *pModel, int frame, float yaw )
{
	...
	else if( frame >= psprite->numframes )
	{
		if( frame > psprite->numframes )
			gEngfuncs_gl.Con_Printf( S_WARN "%s: no such frame %d (%s)\n", __func__, frame, pModel->name );
...
static float R_GetSpriteFrameInterpolant( cl_entity_t *ent, mspriteframe_t **oldframe, mspriteframe_t **curframe )
{
	...
	{
		gEngfuncs_gl.Con_Reportf( S_WARN "%s: no such frame %d (%s)\n", __func__, frame, ent->model->name );
		frame = psprite->numframes - 1;
	}
...
static float R_SpriteGlowBlend( vec3_t origin, int rendermode, int renderfx, float *pscale )
{
	...
	if( RP_NORMALPASS( ))
	{
		tr = gEngfuncs_gl.EV_VisTraceLine( RI.vieworg, origin, r_traceglow.value ? PM_GLASS_IGNORE : (PM_GLASS_IGNORE|PM_STUDIO_IGNORE));
...
void R_DrawSpriteModel( cl_entity_t *e )
{
	...
	if( oldframe == frame )
	{
		...
		glColor4f( color[0], color[1], color[2], tr.blend );
	...
	else
	{
		...
		if( ilerp != 0.0f )
		{
			glColor4f( color[0], color[1], color[2], tr.blend * ilerp );
		...
		if( lerp != 0.0f )
		{
			glColor4f( color[0], color[1], color[2], tr.blend * lerp );
			GL_Bind( XASH_TEXTURE0, frame->gl_texturenum );
			R_DrawSpriteQuad( frame, origin, v_right, v_up, scale );
		}
	}
	...
	if( R_SpriteHasLightmap( e, psprite->texFormat ))
	{
		...
		glColor4f( color2[0], color2[1], color2[2], tr.blend );

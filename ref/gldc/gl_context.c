...
ref_api_t      gEngfuncs_gl;
ref_globals_t *gpGlobals_gl;
ref_client_t  *gp_cl;
ref_host_t    *gp_host;
	...
	if( create )
	{
		switch( mod->type )
		{
			...
			case mod_brush:
				...
				break;
			default: gEngfuncs_gl.Host_Error( "%s: unsupported type %d\n", __func__, mod->type );
		}
	}

	if( loaded && gEngfuncs_gl.drawFuncs->Mod_ProcessUserData )
		gEngfuncs_gl.drawFuncs->Mod_ProcessUserData( mod, create, buf );
...
static void R_ProcessEntData( qboolean allocate, cl_entity_t *entities, unsigned int max_entities )
{
	...
	if( gEngfuncs_gl.drawFuncs->R_ProcessEntData )
		gEngfuncs_gl.drawFuncs->R_ProcessEntData( allocate );
}
...
static qboolean R_SetDisplayTransform( ref_screen_rotation_t rotate, int offset_x, int offset_y, float scale_x, float scale_y )
{
	...
	if( rotate > 0 )
	{
		gEngfuncs_gl.Con_Printf("rotation transform not supported\n");
		ret = false;
	}

	if( offset_x || offset_y )
	{
		gEngfuncs_gl.Con_Printf("offset transform not supported\n");
		ret = false;
	}

	if( scale_x != 1.0f || scale_y != 1.0f )
	{
		gEngfuncs_gl.Con_Printf("scale transform not supported\n");
		ret = false;
	}
...
int EXPORT GetRefAPI( int version, ref_interface_t *funcs, ref_api_t *engfuncs, ref_globals_t *globals )
{
	...
	*funcs = gReffuncs;
	gEngfuncs_gl = *engfuncs;
	gpGlobals_gl = globals;

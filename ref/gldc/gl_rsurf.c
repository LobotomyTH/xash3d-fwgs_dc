...
{
	if( gEngfuncs_gl.drawFuncs->Mod_GetCurrentVis && tr.fCustomRendering )
		return gEngfuncs_gl.drawFuncs->Mod_GetCurrentVis();
	return RI.visbytes;
}

void Mod_SetOrthoBounds( const float *mins, const float *maxs )
{
	if( gEngfuncs_gl.drawFuncs->GL_OrthoBounds )
	{
		gEngfuncs_gl.drawFuncs->GL_OrthoBounds( mins, maxs );
	}
...
static void SubdividePolygon_r( model_t *loadmodel, msurface_t *warpface, int numverts, float *verts )
{
	...
	if( numverts > ( SUBDIVIDE_SIZE - 4 ))
		gEngfuncs_gl.Host_Error( "%s: too many vertexes on face ( %i )\n", __func__, numverts );

	sample_size = gEngfuncs_gl.Mod_SampleSizeForFace( warpface );
...
static void GL_SetupFogColorForSurfacesEx( int passes, float density, qboolean blend_lightmaps )
{
	...
	glFogf( GL_FOG_DENSITY, RI.fogDensity * density );
#endif
}
...
void GL_BuildPolygonFromSurface( model_t *mod, msurface_t *fa )
{
	...
	sample_size = gEngfuncs_gl.Mod_SampleSizeForFace( fa );
...
static void LM_UploadBlock( qboolean dynamic )
{
	...
	else
	{
		...
		if( ++gl_lms.current_lightmap_texture == MAX_LIGHTMAPS )
			gEngfuncs_gl.Host_Error( "%s: full\n", __func__ );
	}
}
...
static void R_BuildLightMap( const msurface_t *surf, byte *dest, int stride, qboolean dynamic )
{
	...
	const int sample_size = gEngfuncs_gl.Mod_SampleSizeForFace( surf );
...
void R_DrawWaterSurfaces( void )
{
	...
	glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
	glColor4f( 1.0f, 1.0f, 1.0f, tr.movevars->wateralpha );
...
static void R_SetRenderMode( cl_entity_t *e )
{
	...
	case kRenderTransAdd:
		glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
		glColor4f( tr.blend, tr.blend, tr.blend, 1.0f );
		glBlendFunc( GL_ONE, GL_ONE );
		glDepthMask( GL_FALSE );
		glEnable( GL_BLEND );
		break;
	case kRenderTransAlpha:
		...
		{
			glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
			glColor4f( 1.0f, 1.0f, 1.0f, tr.blend );
			glEnable( GL_BLEND );
		}
		else
		{
...
void R_DrawBrushModel( cl_entity_t *e )
{
	...
	for( i = 0; i < clmodel->nummodelsurfaces; i++, psurf++ )
	{
		...
		if( num_sorted < gpGlobals_gl->max_surfaces )
		{
			gpGlobals_gl->draw_surfaces[num_sorted].surf = psurf;
			gpGlobals_gl->draw_surfaces[num_sorted].cull = cull_type;
			num_sorted++;
		}
	}
	...
	if( !FBitSet( clmodel->flags, MODEL_LIQUID ) && e->curstate.rendermode == kRenderTransTexture && !gl_nosort.value )
		qsort( gpGlobals_gl->draw_surfaces, num_sorted, sizeof( sortedface_t ), R_SurfaceCompare );

	// draw sorted translucent surfaces
	for( i = 0; i < num_sorted; i++ )
		if( !allow_vbo || !R_AddSurfToVBO( gpGlobals_gl->draw_surfaces[i].surf, true ) )
			R_RenderBrushPoly( gpGlobals_gl->draw_surfaces[i].surf, gpGlobals_gl->draw_surfaces[i].cull );
...
void R_GenerateVBO( void )
{
	...
	{
		gEngfuncs_gl.Cvar_FullSet( "gl_vbo", "0", FCVAR_READ_ONLY );
		return;
	}
		...
		for( j = 0; j < numtextures; j++ )
		{
			...
			for( i = 0; i < numsurfaces; i++ )
			{
				...
				if( vbo->array_len + surf->polys->numverts > VBOINDEX_MAX )
				{
					...
					gEngfuncs_gl.Con_Printf( "%s: allocated array of %d verts, texture %d, lm %d\n", __func__, vbo->array_len, j, k );
	...
	// allocate last array
	vbo->array = Mem_Calloc( vbos.mempool, sizeof( vbovertex_t ) * vbo->array_len );
	gEngfuncs_gl.Con_Printf( "%s: allocated array of %d verts\n", __func__, vbo->array_len );
		...
		for( j = 0; j < numtextures; j++ )
		{
			...
			for( i = 0; i < numsurfaces; i++ )
			{
				...
				if( len + surf->polys->numverts > VBOINDEX_MAX )
				{
					...
					glGenBuffersARB( 1, &vbo->glindex );
					glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbo->glindex );
					glBufferDataARB( GL_ARRAY_BUFFER_ARB, vbo->array_len * sizeof( vbovertex_t ), vbo->array, GL_STATIC_DRAW_ARB );
	...
	// prepare decal array
	glGenBuffersARB( 1, &vbos.decaldata->decalvbo );
	glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.decaldata->decalvbo );
	glBufferDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vbovertex_t ) * DECAL_VERTS_CUT * MAX_RENDER_DECALS, vbos.decaldata->decalarray, GL_DYNAMIC_DRAW_ARB );
	...
	{
		glGenBuffersARB( 1, &vbos.dlight_vbo );
		glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.dlight_vbo );
		glBufferDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vec2_t ) * (int)( vbos.arraylist->next ? VBOINDEX_MAX + 1 : vbos.arraylist->array_len + 1 ), NULL, GL_STREAM_DRAW_ARB );
		glGenBuffersARB( 1, &vbos.decal_dlight_vbo );
		glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.decal_dlight_vbo );
		glBufferDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vbos.decal_dlight ), NULL, GL_STREAM_DRAW_ARB );
	}
...
void R_AddDecalVBO( decal_t *pdecal, msurface_t *surf )
{
	...
	glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.decaldata->decalvbo );
	glBufferSubDataARB( GL_ARRAY_BUFFER_ARB, decalindex * sizeof( vbovertex_t ) * DECAL_VERTS_CUT, sizeof( vbovertex_t ) * numVerts, &vbos.decaldata->decalarray[decalindex * DECAL_VERTS_CUT] );
	glBindBufferARB( GL_ARRAY_BUFFER_ARB, 0 );
...
void R_ClearVBO( void )
{
	...
	for( vbo = vbos.arraylist; vbo; vbo = vbo->next )
		glDeleteBuffersARB( 1, &vbo->glindex );

	vbos.arraylist = NULL;

	if( vbos.decaldata )
		glDeleteBuffersARB( 1, &vbos.decaldata->decalvbo );

	if( vbos.dlight_vbo )
		glDeleteBuffersARB( 1, &vbos.dlight_vbo );

	if( vbos.decal_dlight_vbo )
		glDeleteBuffersARB( 1, &vbos.decal_dlight_vbo );
...
static void R_EnableDetail( void )
{
	...
	if( mtst.details_enabled && mtst.tmu_dt != -1 )
	{
		...
		glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), (void*)offsetof(vbovertex_t, gl_tc ) );
		glMatrixMode( GL_TEXTURE );
		glLoadIdentity();
		glScalef( mtst.glt->xscale, mtst.glt->yscale, 1 );
#else
		glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), (void*)offsetof(vbovertex_t, dt_tc ) );
#endif
	}
}
...
static void R_SetLightmap( void )
{
	...
	glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), (void*)offsetof(vbovertex_t, lm_tc ) );
}
...
static void R_SetupVBOArrayStatic( vboarray_t *vbo, qboolean drawlightmap, qboolean drawtextures )
{
	...
	if( vboarray.astate != VBO_ARRAY_STATIC )
	{
		...
		if( vboarray.astate != VBO_ARRAY_DLIGHT )
		{
			...
			glVertexPointer( 3, GL_FLOAT, sizeof( vbovertex_t ), (void*)offsetof(vbovertex_t,pos) );
		}
...
static void R_SetupVBOArrayDlight( vboarray_t *vbo, texture_t *texture )
{
	...
	if( vboarray.astate != VBO_ARRAY_DLIGHT )
	{
		...
		if( vbos.dlight_vbo  )
			...
			glTexCoordPointer( 2, GL_FLOAT, sizeof( float ) * 2, vbos.dlight_tc );
...
static void R_SetupVBOArrayDecalDlight( int decalcount )
{
	...
	glBufferDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vbovertex_t ) * DECAL_VERTS_MAX * decalcount, vbos.decal_dlight , GL_STREAM_DRAW_ARB );
#endif
	R_SetDecalMode( true );
	...
	{
		glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), &vbos.decal_dlight[0].lm_tc );
		GL_SelectTexture( mtst.tmu_gl );
		glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), &vbos.decal_dlight[0].gl_tc );
		glVertexPointer( 3, GL_FLOAT, sizeof( vbovertex_t ), &vbos.decal_dlight[0].pos);
	}
...
static void R_AdditionalPasses( vboarray_t *vbo, int indexlen, void *indexarray, texture_t *tex, qboolean resetvbo, size_t offset )
{
	...
	if( r_detailtextures.value && r_vbo_detail.value == 1 && mtst.tmu_dt == -1 && tex->dt_texturenum )
	{
		...
		glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ), (void*)(offset + offsetof( vbovertex_t, gl_tc )));

		// apply scale
		glMatrixMode( GL_TEXTURE );
		glLoadIdentity();
		glScalef( glt->xscale, glt->yscale, 1 );

		// draw
#if !defined XASH_NANOGL || defined XASH_WES && XASH_EMSCRIPTEN // WebGL need to know array sizes
		if( glDrawRangeElements )
			glDrawRangeElements( GL_TRIANGLES, 0, vbo->array_len, indexlen, GL_VBOINDEX_TYPE, indexarray );
		else
#endif
		glDrawElements( GL_TRIANGLES, indexlen, GL_VBOINDEX_TYPE, indexarray );
...
static void R_DrawDlightedDecals( vboarray_t *vbo, msurface_t *newsurf, msurface_t *surf, int decalcount, texture_t *texture )
{
	...
	for( decalsurf = newsurf; ( decali < decalcount ) && (!surf ||( decalsurf != surf )); decalsurf = decalsurf->info->lightmapchain )
	{
		for( pdecal = decalsurf->pdecals; pdecal; pdecal = pdecal->pnext )
		{
			...
			glDrawArrays( GL_TRIANGLE_FAN, decali * DECAL_VERTS_MAX, vbos.decal_numverts[decali] );
			decali++;
		}
		newsurf = surf;
...
static void R_FlushDlights( vboarray_t *vbo, int min_index, int max_index, int dlightindex, vboindex_t *dlightarray )
{
	...
	if( vbos.dlight_vbo )
	{
		...
		glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.dlight_vbo );
		glBufferDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vec2_t )* (max_index - min_index), vbos.dlight_tc + min_index, GL_STREAM_DRAW_ARB );
#endif
		glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbo->glindex );
		glVertexPointer( 3, GL_FLOAT, sizeof( vbovertex_t ),  (void*)(min_index* sizeof( vbovertex_t ) + offsetof(vbovertex_t,pos)) );
		GL_SelectTexture( mtst.tmu_gl );
		glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ),  (void*)(min_index * sizeof( vbovertex_t ) + offsetof(vbovertex_t,gl_tc)) );
		if( mtst.details_enabled && mtst.tmu_dt != -1 )
		{
			GL_SelectTexture( mtst.tmu_dt );
			glTexCoordPointer( 2, GL_FLOAT, sizeof( vbovertex_t ),  (void*)(min_index * sizeof( vbovertex_t ) + offsetof(vbovertex_t,gl_tc)) );
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, BLOCK_SIZE, BLOCK_SIZE, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0 );
#endif
	LM_UploadDynamicBlock();
#if !defined XASH_NANOGL || defined XASH_WES && XASH_EMSCRIPTEN // WebGL need to know array sizes
	if( glDrawRangeElements )
		glDrawRangeElements( GL_TRIANGLES, min_index, max_index, dlightindex, GL_VBOINDEX_TYPE, dlightarray );
	else
#endif
	glDrawElements( GL_TRIANGLES, dlightindex, GL_VBOINDEX_TYPE, dlightarray );
#endif
}
...
static void R_AddSurfaceDecalsDlight( msurface_t *surf, int *pdecalcount )
{
	...
	for( pdecal = surf->pdecals; pdecal; pdecal = pdecal->pnext )
	{
		...
		{
			glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.decal_dlight_vbo );
			glBufferSubDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vbovertex_t ) * decalcount * DECAL_VERTS_MAX, sizeof( vbovertex_t )* numVerts, vbos.decal_dlight + decalcount * DECAL_VERTS_MAX );
		}
#endif
...
static void R_DrawVBODlights( vboarray_t *vbo, vbotexture_t *vbotex, texture_t *texture, int lightmap )
{
	...
	if( vbotex->dlightchain )
	{
...
#ifdef MINIMIZE_UPLOAD
			glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.dlight_vbo );
			glBufferDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vec2_t )* (max_index - min_index), NULL, GL_STREAM_DRAW_ARB );
#endif
		}
		else
		{
		...
		for( surf = newsurf = vbotex->dlightchain; surf; surf = surf->info->lightmapchain )
		{
			...
			{
				glBindBufferARB( GL_ARRAY_BUFFER_ARB, vbos.dlight_vbo );
				glBufferSubDataARB( GL_ARRAY_BUFFER_ARB, sizeof( vec2_t ) * (indexbase - min_index), sizeof( vec2_t )* surf->polys->numverts, vbos.dlight_tc + indexbase );
			}
#endif
...
#if !defined XASH_NANOGL || defined XASH_WES && XASH_EMSCRIPTEN // WebGL need to know array sizes
		if( glDrawRangeElements )
			glDrawRangeElements( GL_TRIANGLES, 0, vbo->array_len, vbotex->curindex, GL_VBOINDEX_TYPE, vbotex->indexarray );
		else
#endif
		glDrawElements( GL_TRIANGLES, vbotex->curindex, GL_VBOINDEX_TYPE, vbotex->indexarray );
...
#if !defined XASH_NANOGL || defined XASH_WES && XASH_EMSCRIPTEN // WebGL need to know array sizes
			if( glDrawRangeElements )
				glDrawRangeElements( GL_LINES, 0, vbo->array_len, vbotex->curindex, GL_VBOINDEX_TYPE, vbotex->indexarray );
			else
#endif
				glDrawElements( GL_LINES, vbotex->curindex, GL_VBOINDEX_TYPE, vbotex->indexarray );
...
#if !XASH_DREAMCAST
	glBindBufferARB( GL_ARRAY_BUFFER_ARB, 0 );
	glVertexPointer( 3, GL_FLOAT, VERTEXSIZE * 4, v );
	glTexCoordPointer( 2, GL_FLOAT, VERTEXSIZE * 4, v + 3 );
	if( drawlightmap )
	{
		GL_SelectTexture( mtst.tmu_lm );
		glTexCoordPointer( 2, GL_FLOAT, VERTEXSIZE * 4, v + 5 );
	}
	R_SetDecalMode( true );
	vboarray.astate = VBO_ARRAY_DECAL;
	vboarray.tstate = VBO_TEXTURE_DECAL;
	...
	for( lightmapchain = vbos.decaldata->lm[k]; lightmapchain; lightmapchain = lightmapchain->info->lightmapchain )
	{
		...
		for( pdecal = lightmapchain->pdecals; pdecal; pdecal = pdecal->pnext )
		{
			...
			if( vbos.decaldata->decals[decalindex].numVerts == -1 )
			{
				...
				glDrawArrays( GL_TRIANGLE_FAN, decalindex * DECAL_VERTS_CUT, vbos.decaldata->decals[decalindex].numVerts );
		}
	}
...
loc0:
	...
	if( node->contents < 0 )
	{
		...
		if( pleaf->efrags )
			gEngfuncs_gl.R_StoreEfrags( &pleaf->efrags, tr.realframecount );
...
void R_MarkLeaves( void )
{
	...
	if( RI.viewleaf != NULL )
	{
		...
		if( RI.viewleaf->contents == CONTENTS_EMPTY )
		{
			...
			leaf = gEngfuncs_gl.Mod_PointInLeaf( test, WORLDMODEL->nodes );
		}
		else
		{
			VectorSet( test, RI.pvsorigin[0], RI.pvsorigin[1], RI.pvsorigin[2] + 16.0f );
	...
	gEngfuncs_gl.R_FatPVS( RI.pvsorigin, REFPVS_RADIUS, RI.visbytes, FBitSet( RI.params, RP_OLDVIEWLEAF ), novis );
	if( force && !novis ) gEngfuncs_gl.R_FatPVS( test, REFPVS_RADIUS, RI.visbytes, true, novis );
...
void GL_RebuildLightmaps( void )
{
	...
	if( gEngfuncs_gl.drawFuncs->GL_BuildLightmaps )
	{
		// build lightmaps on the client-side
		gEngfuncs_gl.drawFuncs->GL_BuildLightmaps( );
	}
}
/*
==================
...
void GL_InitRandomTable( void )
{
	...
	for( tu = 0; tu < MOD_FRAMES; tu++ )
	{
		...
		{
			rtable[tu][tv] = gEngfuncs_gl.COM_RandomLong( 0, 0x7FFF );
		}
	}

	gEngfuncs_gl.COM_SetRandomSeed( 0 );
}

...
static void R_DecalUnlink( decal_t *pdecal )
{
	...
	if( pdecal->psurface )
	{
		...
		else
		{
			...
			if( !tmp ) gEngfuncs_gl.Host_Error( "%s: bad decal list\n", __func__ );
...
void R_DecalShoot( int textureIndex, int entityIndex, int modelIndex, vec3_t pos, int flags, float scale )
{
	...
	{
		gEngfuncs_gl.Con_Printf( S_ERROR "Decal has invalid texture!\n" );
		return;
	}
	...
	{
		gEngfuncs_gl.Con_Printf( S_ERROR "Decals must hit mod_brush!\n" );
		return;
	}
...
void DrawSurfaceDecals( msurface_t *fa, qboolean single, qboolean reverse )
{
	...
	if( single )
	{
		...
		if( gl_polyoffset.value )
		{
			...
			glPolygonOffset( -1.0f, -gl_polyoffset.value );
		}
	}
...
int R_CreateDecalList( decallist_t *pList )
{
	...
	if( WORLDMODEL )
	{
		...
		if( gEngfuncs_gl.drawFuncs->R_CreateStudioDecalList )
		{
			total += gEngfuncs_gl.drawFuncs->R_CreateStudioDecalList( pList, total );
		}
	}
...
void R_ClearAllDecals( void )
{
	...
	if( gEngfuncs_gl.drawFuncs->R_ClearStudioDecals )
	{
		gEngfuncs_gl.drawFuncs->R_ClearStudioDecals();
	}
}

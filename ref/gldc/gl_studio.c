...
void R_StudioInit( void )
{
	...
	gEngfuncs_gl.Cvar_FullSet( "r_studio_drawelements", "0", FCVAR_READ_ONLY );
#endif
...
player_info_t *pfnPlayerInfo( int index )
{
	...
	return gEngfuncs_gl.pfnPlayerInfo( index );
}
...
{
	return gEngfuncs_gl.Mod_ForName( model, crash, false );
}
...
static entity_state_t *R_StudioGetPlayerState( int index )
{
	...
	return gEngfuncs_gl.pfnGetPlayerState( index );
}
...
{
	return gEngfuncs_gl.GetDefaultSprite( REF_CHROME_SPRITE );
}
...
static void R_StudioFxTransform( cl_entity_t *ent, matrix3x4 transform )
{
	...
	case kRenderFxHologram:
		if( !gEngfuncs_gl.COM_RandomLong( 0, 49 ))
		{
			int	axis = gEngfuncs_gl.COM_RandomLong( 0, 1 );

			if( axis == 1 ) axis = 2; // choose between x & z
			VectorScale( transform[axis], gEngfuncs_gl.COM_RandomFloat( 1.0f, 1.484f ), transform[axis] );
		}
		else if( !gEngfuncs_gl.COM_RandomLong( 0, 49 ))
		{
			float	offset;
			int	axis = gEngfuncs_gl.COM_RandomLong( 0, 1 );

			if( axis == 1 ) axis = 2; // choose between x & z
			offset = gEngfuncs_gl.COM_RandomFloat( -10.0f, 10.0f );
			transform[gEngfuncs_gl.COM_RandomLong( 0, 2 )][3] += offset;
		}
		break;
...
static void R_StudioMergeBones( cl_entity_t *e, model_t *m_pSubModel )
{
	...
	panim = gEngfuncs_gl.R_StudioGetAnim( m_pStudioHeader, m_pSubModel, pseqdesc );
...
static void R_StudioSetupBones( cl_entity_t *e )
{
	...
	panim = gEngfuncs_gl.R_StudioGetAnim( m_pStudioHeader, RI.currentmodel, pseqdesc );
	R_StudioCalcRotations( e, pos, q, pseqdesc, panim, f );
...
static void R_StudioDynamicLight( cl_entity_t *ent, alight_t *plight )
{
	...
	if(( mv->skycolor_r + mv->skycolor_g + mv->skycolor_b ) != 0 )
	{
		...
		trace = gEngfuncs_gl.CL_TraceLine( vecSrc, vecEnd, PM_WORLD_ONLY );
		if( trace.ent > 0 ) psurf = gEngfuncs_gl.EV_TraceSurface( trace.ent, vecSrc, vecEnd );
		else psurf = gEngfuncs_gl.EV_TraceSurface( 0, vecSrc, vecEnd );
...
static void R_StudioSetupSkin( studiohdr_t *ptexturehdr, int index )
{
	...
	if( m_fDoRemap ) ptexture = gEngfuncs_gl.CL_GetRemapInfoForEntity( RI.currententity )->ptexture;
...
mstudiotexture_t *R_StudioGetTexture( cl_entity_t *e )
{
	...
	if(( phdr = gEngfuncs_gl.Mod_Extradata( mod_studio, e->model )) == NULL )
		return NULL;

	thdr = m_pStudioHeader;
	if( !thdr ) return NULL;

	if( m_fDoRemap ) ptexture = gEngfuncs_gl.CL_GetRemapInfoForEntity( e )->ptexture;
	...
	while(( i = *( ptricmds++ )))
	{
		...
		for( ; i > 0; i--, ptricmds += 4 )
		{
			if( glowShell )
			{
				...
				glTexCoord2f( g_studio.chrome[idx][0] * s, g_studio.chrome[idx][1] * t );
				glVertex3fv( vert );
			}
			else
			{
...
static void R_StudioDrawArrays( uint startverts, uint startelems )
{
	...
	glTexCoordPointer( 2, GL_FLOAT, 0, g_studio.arraycoord );

	if( !( g_nForceFaceFlags & STUDIO_NF_CHROME ) )
	{
		glEnableClientState( GL_COLOR_ARRAY );
		glColorPointer( 4, GL_UNSIGNED_BYTE, 0, g_studio.arraycolor );
	}

#if !defined XASH_NANOGL || defined XASH_WES && XASH_EMSCRIPTEN // WebGL need to know array sizes
	if( glDrawRangeElements )
		glDrawRangeElements( GL_TRIANGLES, startverts, g_studio.numverts,
			g_studio.numelems - startelems, GL_UNSIGNED_SHORT, &g_studio.arrayelems[startelems] );
	else
#endif
		glDrawElements( GL_TRIANGLES, g_studio.numelems - startelems, GL_UNSIGNED_SHORT, &g_studio.arrayelems[startelems] );
...
{
	if( gEngfuncs_gl.CL_EntitySetRemapColors( RI.currententity, RI.currentmodel, newTop, newBottom ))
		m_fDoRemap = true;
}
...
{
	player_info_t	*info = gEngfuncs_gl.pfnPlayerInfo( index );
	player_model_t	*state;

	state = &g_studio.player_models[index];

	// g-cont: force for "dev-mode", non-local games and menu preview
	if(( gpGlobals_gl->developer || !ENGINE_GET_PARM( PARM_LOCAL_GAME ) || !RI.drawWorld ) && info->model[0] )
	{
		if( Q_strcmp( state->name, info->model ))
		{
			...
			if( gEngfuncs_gl.fsapi->FileExists( state->modelname, false ))
				state->model = gEngfuncs_gl.Mod_ForName( state->modelname, false, true );
...
int R_GetEntityRenderMode( cl_entity_t *ent )
{
	...
	if(( phdr = gEngfuncs_gl.Mod_Extradata( mod_studio, model )) == NULL )
	{
...
static void R_StudioClientEvents( void )
{
	...
	{
		dlight_t	*el = gEngfuncs_gl.CL_AllocElight( 0 );
	...
	for( i = 0; i < pseqdesc->numevents; i++ )
	{
		...
		if( (float)pevent[i].frame > start && pevent[i].frame <= end )
			gEngfuncs_gl.pfnStudioEvent( &pevent[i], e );
	}
}
...
static int R_StudioDrawPlayer( int flags, entity_state_t *pplayer )
{
	...
	R_StudioSetHeader((studiohdr_t *)gEngfuncs_gl.Mod_Extradata( mod_studio, RI.currentmodel ));
	...
	if( flags & STUDIO_RENDER )
	{
		...
		if( !( !gpGlobals_gl->developer && gp_cl->maxclients == 1 ) && ( RI.currentmodel == RI.currententity->model ))
		...
		if( pplayer->weaponmodel )
		{
			...
			m_pStudioHeader = (studiohdr_t *)gEngfuncs_gl.Mod_Extradata( mod_studio, pweaponmodel );
...
void R_GatherPlayerLight( void )
{
	...
	gEngfuncs_gl.SetLocalLightLevel( ( c.r + c.g + c.b ) / 3 );
}
...
{
	return gEngfuncs_gl.Mod_CacheCheck( c );
}

static void *pfnMod_StudioExtradata( model_t *mod )
{
	return gEngfuncs_gl.Mod_Extradata( mod_studio, mod );
}

static void pfnMod_LoadCacheFile( const char *path, struct cache_user_s *cu )
{
	gEngfuncs_gl.Mod_LoadCacheFile( path, cu );
}

static cvar_t *pfnGetCvarPointer( const char *name )
{
	return (cvar_t*)gEngfuncs_gl.pfnGetCvarPointer( name, 0 );
}

static void *pfnMod_Calloc( int number, size_t size )
{
	return gEngfuncs_gl.Mod_Calloc( number, size );
}
...
void CL_InitStudioAPI( void )
{
	...
	cl_righthand = gEngfuncs_gl.pfnGetCvarPointer( "cl_righthand", 0 );

	// Xash will be used internal StudioModelRenderer
	if( gEngfuncs_gl.pfnGetStudioModelInterface( STUDIO_INTERFACE_VERSION, &pStudioDraw, &gStudioAPI ))
		return;

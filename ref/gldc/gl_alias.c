...
static rgbdata_t *Mod_CreateSkinData( model_t *mod, const byte *data, int width, int height )
{
	...
	if( !gEngfuncs_gl.Image_CustomPalette() )
	{
	...
	// make an copy
	return gEngfuncs_gl.FS_CopyImage( &skin );
}
...
static const void *Mod_LoadSingleSkin( model_t *loadmodel, const daliasskintype_t *pskintype, int skinnum, int size )
{
	...
	if( !gEngfuncs_gl.fsapi->FileExists( checkname, false ) || ( pic = gEngfuncs_gl.FS_LoadImage( checkname, NULL, 0 )) == NULL )
...
=================
*/
void _Mod_LoadAliasModel( model_t *mod, const void *buffer, qboolean *loaded )
{
...
static void R_AliasDynamicLight( cl_entity_t *ent, alight_t *plight )
{
	...
	if(( mv->skycolor_r + mv->skycolor_g + mv->skycolor_b ) != 0 )
	{
		...
		trace = gEngfuncs_gl.CL_TraceLine( vecSrc, vecEnd, PM_STUDIO_IGNORE );
...
static void GL_DrawAliasFrame( aliashdr_t *paliashdr )
{
	...
	while( 1 )
	{
		...
		do
		{
			...
			glColor4f( g_alias.lightcolor[0] * lv_tmp, g_alias.lightcolor[1] * lv_tmp, g_alias.lightcolor[2] * lv_tmp, tr.blend );
...
void R_DrawAliasModel( cl_entity_t *e )
{
	...
	//
	m_pAliasHeader = (aliashdr_t *)gEngfuncs_gl.Mod_Extradata( mod_alias, RI.currententity->model );
	if( !m_pAliasHeader ) return;
	...
	{
		glTranslatef( m_pAliasHeader->scale_origin[0], -m_pAliasHeader->scale_origin[1], m_pAliasHeader->scale_origin[2] );
		glScalef( m_pAliasHeader->scale[0], -m_pAliasHeader->scale[1], m_pAliasHeader->scale[2] );
	}
	else
	{
		glTranslatef( m_pAliasHeader->scale_origin[0], m_pAliasHeader->scale_origin[1], m_pAliasHeader->scale_origin[2] );
	...
	if( m_fDoRemap ) pinfo = gEngfuncs_gl.CL_GetRemapInfoForEntity( e );
